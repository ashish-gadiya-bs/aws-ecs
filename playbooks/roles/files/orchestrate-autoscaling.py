...
#@# TODO:
# accept Memory &  CPU limits from arguments.
# Hardcoded for now
parser.add_argument('-sc',action='store_true',help='OPTIONAL: Specify if CPU based autoscaling is needed')
parser.add_argument('-sm',action='store_true',help='OPTIONAL: Specify if Memory based autoscaling is needed')
...
def setupScalableTarget(mode,serviceName,clusterName,roleArn="arn:aws:iam::953030164212:role/ecs-application-autoscale"):
    ...
    if mode:
        print "Setting up scalable target"
...
def setupServiceScaleoutMem(mode,serviceName,clusterName,roleArn="arn:aws:iam::953030164212:role/ecs-application-autoscale"):
    ...
    if mode:
        print "Setting up service scaleout target - Memory"
    client = boto3.client('application-autoscaling',region_name=region)
...
def setupServiceScaleoutCPU(mode,serviceName,clusterName,roleArn="arn:aws:iam::953030164212:role/ecs-application-autoscale"):
    """Function to setup scaling policies target"""
    if mode:
        print "Setting up service scaleout target - CPU"
    client = boto3.client('application-autoscaling',region_name=region)
    response = client.put_scaling_policy(
...
def setupServiceScaleinMem(mode,serviceName,clusterName,roleArn="arn:aws:iam::953030164212:role/ecs-application-autoscale"):
    """Function to setup scaling policies target"""
    if mode:
        print "Setting up service scalein target - Mem"
    client = boto3.client('application-autoscaling',region_name=region)
    response = client.put_scaling_policy(
...
def setupServiceScaleinCPU(mode,serviceName,clusterName,roleArn="arn:aws:iam::953030164212:role/ecs-application-autoscale"):
    """Function to setup scaling policies target"""
    if mode:
        "Setting up service scalein target - CPU"
    client = boto3.client('application-autoscaling',region_name=region)
    response = client.put_scaling_policy(
      PolicyName=serviceName+'-Low-CPU',
      PolicyType='StepScaling',
      ResourceId="service/"+clusterName+"/"+serviceName,
      ScalableDimension='ecs:service:DesiredCount',
      ServiceNamespace='ecs',
    ...
    setupLowCPUAlarm(serviceName,clusterName,policyArnValue)
...
def setupLowCPUAlarm(serviceName,clusterName,Arn):
    client = boto3.client('cloudwatch')
    response = client.put_metric_alarm(
      AlarmName=serviceName+'-LowCPU-Alarm',
      ComparisonOperator='GreaterThanThreshold',
      EvaluationPeriods=2,
      MetricName='CPUUtilization',
      Namespace='AWS/ECS',
      ...
      AlarmDescription='Alarm when server CPU drops below 20%',
      Dimensions=[
          {
            'Name': 'ClusterName',
            'Value': clusterName
...
def createService(mode,confFile,region,count,clusterName,cpuScaling,memScaling):
           ...
           for svc in modules['serviceCreate']:
               ...
               try:
                  ...
                  if createResponse == 200:
                     # Setup autoscaling based on CPU or Memory scale
                     # If none is requested, move on
                     if cpuScaling:
                         setupScalableTarget(mode,svcServiceName,svcCluster)
                         setupServiceScaleoutCPU(mode,svcServiceName,svcCluster)
                         setupServiceScaleinCPU(mode,svcServiceName,svcCluster)
                     if memScaling:
                         print "Scaling for memory"
                         setupScalableTarget(mode,svcServiceName,svcCluster)
                         setupServiceScaleoutMem(mode,svcServiceName,svcCluster)
                         setupServiceScaleinMem(mode,svcServiceName,svcCluster)
                     # verbose
                     if mode:
...
def main(mode,confFile,region,count,action,cpuScaling=False,memScaling=False):
            ...
            if isinstance(svc['cluster'],list):
               ...
               if action == 'create':
                  for cluster in svc['cluster']:
                      print cluster
                      response = createService(mode,serviceConfFile,region,count,cluster,cpuScaling,memScaling)
               ...
               if action == 'create':
                      response = createService(mode,serviceConfFile,region,count,svc['cluster'],cpuScaling,memScaling)
                      responseCode = response
               if action == 'update':
...
if __name__ == '__main__':
   ...
   if args.create:
      action = 'create'
      # check if autoscaling is required
      if args.sc:
          cpuScaling = True
      else:
          cpuScaling = False
      ...
      if confFile:
         ...
         if mode:
            print "Service config file : %s " %(serviceConfFile)
         # Create service for a single file
         main(mode,serviceConfFile,region,count,action,cpuScaling,memScaling)
      else:
         # loop over dir and extract all files
         for confFile in os.listdir(os.path.dirname(os.path.abspath(__file__))+"/config/"+args.env+"/"):
             if confFile.endswith(".yaml"):
